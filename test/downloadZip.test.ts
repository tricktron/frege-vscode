...
describe('Given frege server github release url', () => 
{
	it('Then it downloads and extracts the frege server tar', async () => 
    {
		...
		assert.ok(existsSync(path.join(
            downloadDir, 
            getFregeStartScriptPath(fregeServerName, version)
            ))
        );
	}).timeout(10000);
});
...
describe('Given frege server name and the frege version', () => 
{
    ...
    {
		const expectedStartScriptPath = 
            path.normalize(`${fregeServerName}-${version}/bin/${fregeServerName}`);
		assertSystemPath(
            getFregeStartScriptPath(fregeServerName, version), 
            expectedStartScriptPath
        );
	});
	it('Then it can create the github release url to the frege tar archive dynamically', () => 
    {
		const expectedDownloadUrl = 
            `https://github.com/tricktron/${fregeServerName}/releases/download/v${version}/${fregeServerName}-${version}.tar`;
		assert.strictEqual(
            getFregeTarGithubUrl(fregeServerName, version), 
            expectedDownloadUrl);
	});
});
